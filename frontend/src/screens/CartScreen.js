...
import { addToCart, refreshCart, removeFromCart } from '../actions/cartActions';
...
const CartScreen = () => {
    ...
    return (
        <>
            <Meta title='User Cart | UnityShop' />
            <Row className='mt-3'>
                <Col lg={8}>
                            ...
                            {cartItems.map((item) =>
                                <ListGroup.Item key={item.product} className='shadow rounded-2 mb-3'>
                                    <Row className='justify-content-between align-items-center'>
                                        <Col md={2} xs={4}>
                                            <Image src={`${STATIC_BASE_URL}${item.image}`} alt={item.name} fluid rounded style={{ height: '100px' }} />
                                        </Col>
                                        ...
                                        <Col md={5} xs={8} className='d-flex flex-column align-items-start m-0 letter-spacing-0'>
                                            ...
                                            <Col md={2} as='div' className='m-0 letter-spacing-1 d-flex w-100'>
                                                {item.discount
                                                    ? (
                                                        <>
                                                            <h2 className='product-price transi'>
                                                                <Price price={item.price - (item.price * (item.discount / 100))} />
                                                            </h2>
                                                            <del
                                                                className='product-price text-secondary px-2 d-flex align-items-center'
                                                                style={{ fontSize: '12px' }}>
